...
create table products_user AS
SELECT products.id, products.product_name, tabla.product,  tabla.user_id, tabla.count_product
FROM   products
JOIN   (SELECT 
            CAST(SUBSTRING_INDEX(
                SUBSTRING_INDEX(transactions.product_ids, ',', product_id), ',', -1
            ) AS UNSIGNED) AS product,  transactions.user_id,
            COUNT(*) AS count_product
        FROM 
            transactions 
        CROSS JOIN 
            (SELECT id AS product_id FROM products) AS numbers 
        WHERE 
            product_id <= (LENGTH(transactions.product_ids) - LENGTH(REPLACE(transactions.product_ids, ',', '')) + 1) 
        GROUP BY 
            product, user_id
        ORDER BY 
            product
...
ON 
    tabla.product = products.id;
    
select sum(count_product) from products_user
